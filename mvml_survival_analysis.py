...
def do_base_learning(mvml_model, x_RNASeq_batch, x_miRNA_batch, R_matrix_batch, ystatus_batch,
    ...
    # inner_optimizer = torch.optim.Adam([{'params': new_mvml_model.parameters()}], lr=TRAIN_LR, weight_decay=REG_SCALE)
    ...
    for _ in range(TRAIN_N_INNER):
        ...
        for i in range(real_batch_size):
            j = random.randint(0, real_batch_size - 1)
            same_sim = torch.sum(torch.mul(feature_RNASeq[i], feature_miRNA[i])) / torch.mul(
                norm_gene[i], norm_miRNA[i])
            diff_sim = torch.sum(torch.mul(feature_RNASeq[i], feature_miRNA[j])) / torch.mul(
                norm_gene[i], norm_miRNA[j])
            cos_loss = cos_loss + torch.max(torch.tensor(0.0), 0.5 - (same_sim - diff_sim))
        cos_loss = cos_loss / real_batch_size
    ...
    with open('nohup_loss' + '.log', 'a') as f:
        f.writelines("loss_mvml:" + str(loss_mvml) + '\n')
...
def do_final_learning(mvml_model, x_RNASeq_test_support, x_miRNA_test_support, ytime_test_support,
    ...
    for i in range(ytime_test_support.shape[0]):
        for j in range(ytime_test_support.shape[0]):
            R_matrix_batch[i, j] = ytime_test_support[j] >= ytime_test_support[i]
...
def do_final_eval(trained_model, x_RNASeq_test_query, x_miRNA_test_query, ytime_test_query, ystatus_test_query):
    ...
    cind = CIndex(pred_batch_test, ytime_test_query, np.asarray(ystatus_test_query))
...
def meta_learn(mvml_model, x_RNASeq_train, x_miRNA_train, ytime_train, ystatus_train, TRAIN_ITER, TRAIN_OUTER_LR,
               TRAIN_INNER_LR, TRAIN_N_INNER, TRAIN_BATCH_N, TRAIN_SHOTS_N, model_parameters_path, x_RNASeq_target,
               x_miRNA_target, ytime_target, ystatus_target, Target_DEAD_SHOTS_N, Target_ALIVE_SHOTS_N,
               Target_TASK_NUMBER, REG_SCALE):
    ...
    for _ in range(Target_TASK_NUMBER):
        ...
        ind_target_dead_test = random.sample(x_RNASeq_target_dead_index, int(x_RNASeq_target_dead.shape[0] * target_test_rate))
        ...
        ind_target_alive_test = random.sample(x_RNASeq_target_alive_index, int(x_RNASeq_target_alive.shape[0] * target_test_rate))
        ...
        x_RNASeq_target_train_list.append(np.concatenate((x_RNASeq_target_train_dead, x_RNASeq_target_train_alive), axis=0))
        x_miRNA_target_train_list.append(np.concatenate((x_miRNA_target_train_dead, x_miRNA_target_train_alive), axis=0))
        ystatus_target_train_list.append(np.concatenate((ystatus_target_train_dead, ystatus_target_train_alive), axis=0))
        ...
        target_rest_train_index_dead = [n for n in x_RNASeq_target_train_dead_index if n not in target_ind_dead_shots]
        ...
        target_rest_train_index_alive = [n for n in x_RNASeq_target_train_alive_index if n not in target_ind_alive_shots]
    ...
    for t in range(TRAIN_ITER):
        ...
        for i in range(ytime_batch.shape[0]):
            for j in range(ytime_batch.shape[0]):
                R_matrix_batch[i, j] = ytime_batch[j] >= ytime_batch[i]
        ...
        for i in range(Target_TASK_NUMBER):
            CI = do_final_learning(mvml_model, x_RNASeq_support_list[i], x_miRNA_support_list[i], ytime_support_list[i],
                                   ystatus_support_list[i], x_RNASeq_target_test_list[i], x_miRNA_target_test_list[i],
                                   ytime_target_test_list[i], ystatus_target_test_list[i], TRAIN_OUTER_LR, REG_SCALE, META_STEP=5)
    ...
    with open('Cindex' + '.log', 'a') as f:
        f.writelines("Ave C_index:" + str(np.max(np.array(cind_list_iter_max))) + '\n')
...
if __name__ == '__main__':
    ...
    meta_learn(mvml_model=mvml_model, x_RNASeq_train=x_RNASeq_train, x_miRNA_train=x_miRNA_train,
               ...
               ystatus_target=ystatus_target,
               Target_DEAD_SHOTS_N=Target_DEAD_SHOTS_N, Target_ALIVE_SHOTS_N=Target_ALIVE_SHOTS_N,
               Target_TASK_NUMBER=Target_TASK_NUMBER, model_parameters_path=model_parameters_path, REG_SCALE=REG_SCALE)